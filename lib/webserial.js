...
class Transport {
    ...
    get_info() {
        if (!this.info) return '';
        return this.info.usbVendorId && this.info.usbProductId
            ? `WebSerial VendorID 0x${this.info.usbVendorId.toString(16)} ProductID 0x${this.info.usbProductId.toString(16)}`
            : "";
    }
    get_pid() {
        if (!this.info) return '';
        return this.info.usbProductId;
    }
    slip_writer(data) {
        let count_esc = 0;
        let i = 0, j = 0;
    ...
    async write(data) {
        ...
        if (this.device.writable) {
            ...
            await new Promise((res, rej) => {
                this.device.write(out_data, (err) => {
                    console.log('asdfasfdsaf-----', err);
                    if (err) rej(err);
                    else res('success');
                });
            });
    ...
    async setDTR(state) {
        ...
        this.device.set({ dtr: state });
    }
    async connect(baud = 115200) {
        // need to change
        this.device.update({ baudRate: baud });
